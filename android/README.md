...
- **`engine/`** — the entire game simulation as plain Kotlin (no Android
  ...
  the run, an exposed-back hit destroys the obstacle, a shield charge turns
  either into a bounce-off), pickups, scoring, difficulty ramps. Fully
  unit-tested with JUnit 5 (`./gradlew :engine:test`) — this is the part
  correctness bugs would actually live in, and it runs on any JVM without
  an emulator.
...
actually be compiled and tested here (`./gradlew :engine:test` — 19/19
